...
def initialize_project(
    ...
    if os.path.exists("pyproject.toml"):
        replacements = [
            (
                ...
                f'"Homepage" = "https://github.com/{github_username}/{project_name}/"',
            ),
            (
                '"Bug Tracker" = "https://github.com/OSpoon/starter_pypkg/issues"',
                f'"Bug Tracker" = "https://github.com/{github_username}/{project_name}/issues"',
            ),
    ...
    if os.path.exists("README.md"):
        replacements = [
            ...
            ("OSpoon", github_username),
            (
                "https://github.com/OSpoon/starter_pypkg",
                f"https://github.com/{github_username}/{project_name}",
            ),
    ...
    if os.path.exists("src/starter_pypkg"):
        ...
        if os.path.exists(f"src/{package_name}/starter_pypkg.py"):
            os.rename(
                f"src/{package_name}/starter_pypkg.py",
    ...
    if os.path.exists("tests/test_starter_pypkg.py"):
        replacements = [
            (
                ...
                f"from src.{package_name} import add",
            ),
        ]
    ...
    if reinit_git:
        ...
        try:
            ...
            if os.path.exists(".git"):
                shutil.rmtree(".git")
            subprocess.run([git_path, "init"], check=True)  # noqa: S603
            subprocess.run([git_path, "add", "."], check=True)  # noqa: S603
            ...
            with gr.Row(equal_height=True):
                with gr.Column():
                    project_name = gr.Textbox(
                        label="项目名称 (python包名, 例如: my_package)",
                        value=os.path.basename(os.getcwd()),
                    )
                    display_name = gr.Textbox(
                        label="项目显示名称 (用于README等文档)",
                        info="留空将使用项目名称",
                    )
                ...
                with gr.Column():
                    ...
                    copyright_year = gr.Textbox(
                        label="版权年份", value=str(datetime.datetime.now().year)
                    )
